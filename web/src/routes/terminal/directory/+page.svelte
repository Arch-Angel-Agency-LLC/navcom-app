...
<svelte:head>
  <title>Cached directory · Field Terminal</title>
  <meta name="description" content="What this device holds when there is no watch." />
</svelte:head>

<header>
  <p class="eyebrow"><a href="/terminal/">← Status</a></p>
  <h1>Cached directory</h1>
</header>

<section>
  {#if watch.state.state === 'dark'}
    <p>
      No watch. This is what this device is holding, and it is all you have — so it is worth
      knowing how old it is.
    </p>
  {:else}
    <p>
      There is a watch up. <strong><a href="/terminal/query/">Ask it</a></strong> — someone
      with both hands free will do the lookup, and can answer things this list cannot.
    </p>
    <p class="cost">This is what you have when nobody is watching. It is worse, on purpose.</p>
  {/if}
</section>
